...
sidebar_position: 7
title: "Email"
description: "Set up Hermes Agent as an email assistant via SMTP and optional IMAP"
---
...
- **SMTP access** on the email account
- **IMAP enabled** on the email account if you want inbound email polling
...
Select **Email** from the platform menu. The wizard prompts for your email address, password, SMTP host, optional IMAP host, and allowed senders.
...
1. Tests SMTP
2. If `EMAIL_IMAP_HOST` is configured, tests IMAP
3. If IMAP is configured, marks all existing inbox messages as "seen" and starts polling for new messages
...
| **Provider blocks IMAP basic auth** | Remove `EMAIL_IMAP_HOST` and use SMTP-only mode. Hermes can still send to `EMAIL_HOME_ADDRESS` and explicit email targets, but it will not poll inbound mail. |
